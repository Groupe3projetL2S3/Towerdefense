...
int main(int argc, char* argv[])
{
  ...
  while (!gameover)
    {
      ...
      /* look for an event */
      update_events(key,&liste_mob, &liste_tower, creep, zombie, ender, sniper1, sniper2, magic1, magic2, map, map_objet, &num_mob, &case1, &case2, &case3, &case4, &gameover);
  ...
  if (sniper2.up.up != NULL)
    SDL_FreeSurface(sniper2.up.up);
  if (sniper1.sell.sell != NULL)
    SDL_FreeSurface(sniper1.sell.sell);
  if (sniper2.sell.sell != NULL)
    SDL_FreeSurface(sniper2.sell.sell);
  if (magic1.range.range != NULL)
    SDL_FreeSurface(magic1.range.range);
  if (magic2.range.range != NULL)
    SDL_FreeSurface(magic2.range.range);
  if (magic1.up.up != NULL)
    SDL_FreeSurface(magic1.up.up);
  if (magic2.up.up != NULL)
    SDL_FreeSurface(magic2.up.up);
  if (magic1.sell.sell != NULL)
    SDL_FreeSurface(magic1.sell.sell);
  if (magic2.sell.sell != NULL)
    SDL_FreeSurface(magic2.sell.sell);
